...
def geraChavePrivada(tamanho : int, arquivo : str = None):
...
def geraChavePublica(private_key : object, arquivo=None):
...
def converteChavePublica(public_pem ) -> object:
    public_key = serialization.load_pem_public_key(public_pem)
    return public_key

# Cifra bytes com a chave pública. Se plaintext for uma string ela é convertida para bytes.
def cifraComPublica(plaintext, public_key : object):
...
def decifraComPrivada(ciphertext, private_key : object, text : bool =True):
